...
export default function Home() {
  ...
  const splitContentEqually = (htmlContent: string, hasImage: boolean) => {
    const paragraphs = htmlContent.match(/<(p|h3)[\s\S]*?<\/\1>/gi) || [htmlContent];

    if (paragraphs.length <= 1) {
      return { col1: htmlContent, col2: '' };
    }

    const totalLen = paragraphs.reduce((acc, p) => acc + p.replace(/<[^>]+>/g, '').length, 0);
    const targetLen = totalLen * (hasImage ? 0.44 : 0.50);

    let acc = 0;
    let splitIdx = 1;

    for (let i = 0; i < paragraphs.length; i++) {
      acc += paragraphs[i].replace(/<[^>]+>/g, '').length;
      if (acc >= targetLen && i < paragraphs.length - 1) {
        splitIdx = i + 1;
        break;
      }
    }
    ...
    return {
      col1: paragraphs.slice(0, splitIdx).join(''),
      col2: paragraphs.slice(splitIdx).join(''),
    };
  };
              ...
              <div className="bg-blue-50/70 border border-blue-200 rounded-xl p-3.5 text-neutral-800">
                ...
                <ol className="space-y-1.5 list-none font-medium">
                  <li className="flex items-start gap-1.5">
                    ...
                    <span>스크랩하고 싶은 뉴스의 페이지 주소(URL)를 추가하여 주세요.(네이버와 같은 포털사이트내 뉴스페이지 이용 권고)</span>
                  </li>
                  <li className="flex items-start gap-1.5">
        ...
        <div className="lg:col-span-8 flex flex-col items-center gap-3">
          <div className="w-full flex justify-center overflow-x-auto p-1">
            ...
            >
                ...
                {activePageArticles.length === 0 ? (
                  ...
                  >
                    {activePageArticles.map((art) => {
                      ...
                      const flexWeight = count === 2 ? Math.max(1, Math.round(plainLen / 200)) : 1;

                      let fontSize = '0.74rem';
                      let lineHeight = '1.40';
                      let titleSize = '1.05rem';

                      if (count === 1) {
                        fontSize = totalCharsOnPage < 1400 ? '0.80rem' : '0.76rem';
                        lineHeight = '1.50';
                        titleSize = '1.20rem';
                      } else if (count === 2) {
                        fontSize = totalCharsOnPage < 1800 ? '0.75rem' : '0.72rem';
                        lineHeight = '1.38';
                        titleSize = '1.02rem';
                      } else {
                        fontSize = '0.68rem';
                      ...
                      const { col1, col2 } = splitContentEqually(art.content, !!art.image);

                      return (
                        <article
                          key={art.id}
                          style={{ flex: `${flexWeight} 1 0%` }}
                          className="flex flex-col border-b border-neutral-300 pb-1.5 last:border-b-0 overflow-hidden min-h-0"
                        >
                          <div className="flex items-baseline gap-2 mb-0.5 flex-wrap">
                            <h3
                              ...
                              className="newspaper-font font-bold text-neutral-950 leading-tight outline-none focus:bg-neutral-50 tracking-tight inline"
                              style={{ fontSize: titleSize }}
                            >
                              {art.title}
                            </h3>
                            {formattedDate && (
                              <span className="text-[7.5px] text-neutral-500 font-normal tracking-normal newspaper-font">
                                ({formattedDate})
                              </span>
                            )}
                          </div>
                          ...
                          {art.subheading && (
                            <div
                              ...
                              className="newspaper-font font-semibold text-neutral-700 bg-neutral-50 border-l-2 border-neutral-500 px-2 py-0.5 mb-1 outline-none focus:bg-neutral-100 whitespace-pre-line tracking-tight"
                              style={{ fontSize: '0.70rem', lineHeight: '1.25' }}
                            >
                              {art.subheading}
                            </div>
                          )}
                          ...
                          {isTwoCol ? (
                            <div 
                              className="newspaper-font newspaper-text flex-1 grid grid-cols-2 gap-3 outline-none focus:bg-neutral-50 overflow-hidden items-start"
                              style={{ fontSize, lineHeight }}
                            >
                              <div className="flex flex-col min-w-0">
                                {art.image && (
                                  <div className="w-full mb-1 p-0.5 border border-neutral-200 bg-white flex justify-center">
                                    <img
                                      src={art.image}
                                      alt="기사 사진"
                                      className="max-h-24 w-auto object-contain block"
                                    />
                                  </div>
                                )}
                                <div
                            ...
                            <div 
                              className="newspaper-font newspaper-text flex-1 outline-none focus:bg-neutral-50 overflow-hidden"
                              style={{ fontSize, lineHeight }}
                            >
                              {art.image && (
              ...
              <div className="border-t border-neutral-300 pt-0.5 mt-0.5 flex justify-between items-center text-[7.5px] text-neutral-400 newspaper-font">
